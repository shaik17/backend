...
const passwordService = require("../services/passwordReset.service");
...
module.exports.forgotPassword = (req, res, next) => {
  ...
  try {
    return profileService
      ...
      .then(async (response) => {
        const data = await passwordService.generateResetCode(response);
        const url = `${config.extras.app_url}/auth/resetPassword?code=${data.resetCode}&userId=${data.userId}`;
        passwordService.triggerResetEmail(url, response);
        return res.status(httpStatus.OK).json({
          msg: "You will receive an email, if you have account already",
        });
      })
      .catch((e) => {
        const code = `Err${CONSTANTS.MODULE_CODE.USER}${CONSTANTS.OPERATION_CODE.POST}${CONSTANTS.ERROR_TYPE.SYSTEM}RESET`;
        return next(
          new APIError(e.message, e.status, true, res.__("system_error"), code)
        );
      });
...
module.exports.resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!data || !data.resetId) {
      const code = `Err${CONSTANTS.MODULE_CODE.USER}${CONSTANTS.OPERATION_CODE.PATCH}${CONSTANTS.ERROR_TYPE.GENERIC}RESET`;
      return next(
        new APIError(
          "Request not found",
          httpStatus.NOT_FOUND,
    ...
    const encryptedValue = userService.generatePassword(body.newPassword);
    const userUpdate = await userService.updateUser(
      { userId: body.userId, isActive: true },
      { $set: encryptedValue }
    );
    if (userUpdate) {
    ...
    return res
      .status(httpStatus.OK)
      .send({ error: "Account is either not available or not yet activated" });
  } catch (e) {
